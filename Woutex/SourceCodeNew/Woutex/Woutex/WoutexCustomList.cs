            ...
            if (context.FunctionName.Equals("ChangeContent", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    var authString = Convert.ToBase64String(
                        Encoding.UTF8.GetBytes(data.Properties["UserName"] + ":" + data.Properties["Password"]));
                    ...
                    var content = new StringContent(context.Values[1].StringValue, System.Text.Encoding.UTF8,
                        "application/json");
                    HttpResponseMessage response = client
                        .PutAsync(BaseURL + "flex/v2/displays/" + DisplayID + "/content", content).Result;
                    var responseBody = response.Content.ReadAsStringAsync().Result;

                    if (response.IsSuccessStatusCode)
                    {
                        this.Log?.Info($"Display {DisplayID} succesfully set to content " +
                                       context.Values[1].StringValue);
                    }
                    else
                    {
            ...
            else if (context.FunctionName.Equals("ResetContent", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    HttpResponseMessage response =
                        client.DeleteAsync(BaseURL + "flex/v2/displays/" + DisplayID + "/content").Result;
                    var responseBody = response.Content.ReadAsStringAsync().Result;
